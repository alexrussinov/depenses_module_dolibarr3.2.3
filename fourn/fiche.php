...
if ( $societe->fetch($socid) )
{
	...
	if ($user->rights->fournisseur->facture->lire)
	{
		...
		//Block for invoices of charges
		$sql = 'SELECT f.rowid,f.ref as facnumber,f.fk_statut,f.dated as df,f.total_ttc as amount,f.paye,';
		$sql.= ' SUM(pf.amount) as am';
		$sql.= ' FROM '.MAIN_DB_PREFIX.'depenses as f';
		$sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiementdepenses_facturedep as pf ON f.rowid=pf.fk_facturefourn';
		$sql.= ' WHERE f.fk_soc = '.$societe->id;
		$sql.= ' GROUP BY f.rowid,f.ref,f.fk_statut,f.dated,f.total_ttc,f.paye';
		$sql.= ' ORDER BY f.dated DESC';
		$resql=$db->query($sql);
		if ($resql)
		{
			$langs->load('depenses');
			...
			if ($num > 0)
			{
				print '<tr class="liste_titre">';
				print '<td colspan="4">';
				print '<table class="nobordernopadding" width="100%"><tr><td>'.$langs->trans('LastSuppliersBillsCategorieDepenses',($num<=$MAXLIST?"":$MAXLIST)).'</td><td align="right"><a href="facture/index.php?socid='.$societe->id.'">'.$langs->trans('AllBills').' ('.$num.')</td>';
				print '<td width="20px" align="right"><a href="'.DOL_URL_ROOT.'/compta/facture/stats/index.php?mode=supplier&socid='.$societe->id.'">'.img_picto($langs->trans("Statistics"),'stats').'</a></td>';
				print '</tr></table>';
				print '</td></tr>';
			}
