...
require_once(DOL_DOCUMENT_ROOT.'/rapports/class/tvapdf.class.php');
require_once(DOL_DOCUMENT_ROOT.'/factures_depenses/lib/tax.lib.php');
...
for ($m = 1 ; $m < 13 ; $m++ )
{
	...
	$date_arr[]=array(dol_print_date(dol_mktime(0,0,0,$m,1,$y),"%b %Y"));
	...
	$tva->tvacoll[]=$x_pay;// by Allex
	
	}
	create_document_tva($tva, $date_arr,$y);
...
function create_document_tva($obj, $date_arr,$y) //By Alex
{
//$h=10;
ob_end_clean(); 
$pdf = new Tvapdf();
...
$pdf->Writefile2($obj,$date_arr);
$file="Rapport_tva".strftime("%d-%m-%Y").".pdf";
$pdf->Output($file, D);
if(file_exists($file))
{
print '<a href="'.DOL_DOCUMENT_ROOT.'/rapports/documents.php?path='.$file.'">'.ShowFileName($file).'.pdf</a>';
}
...
for ($m = 1 ; $m < 13 ; $m++ )
{
	$coll_listsell = vat_by_date($db, $y, 0, 0, 0, $modetax, 'sell', $m);
	$coll_listbuy = vat_by_date($db, $y, 0, 0, 0, $modetax, 'buy', $m);
	$coll_dep=get_tva_depenses($db,$y, $m);
	if (! is_array($coll_listbuy) && $coll_listbuy == -1)
	{
		$langs->load("errors");
		print '<tr><td colspan="5">'.$langs->trans("ErrorNoAccountancyModuleLoaded").'</td></tr>';
	...
	foreach($coll_listsell as $vatrate=>$val)
	{
		...
		$x_coll+=$val['vat'];
		//$tva->tvasell[]=$val['vat']; //Alex
		//print $vatrate; print '&nbsp';
		//print $val['vat']; print '<br>';
	}
	
	$x_dep=0;
	foreach ($coll_dep as $v)
	{
		$x_dep+=$v;
	}
	...
	$x_paye+=$x_dep; // by Alex
	$tva->tvacoll[]=$x_pay;// by Alex
...
print '</table>';
print '<br>';
print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?action=builddoc&amp;year='.$y.'">Create document</a>'; //by Alex
//if(GETPOST('action')=='builddoc') create_document_tva($tva, $date_arr);// by Alex
